...
def transferir():
    if conta_t.get() == '' or valor_t.get() == '' or senha_t.get() == '':
        messagebox.showerror('Um erro foi encontrado!', 'Ops! Preencha todos os dados requeridos.')
    elif senha_t.get() != clienteAtual['senha']:
        messagebox.showerror('Um erro foi encontrado!', 'Senha incorreta. Tente novamente...')
    elif float(valor_t.get()) > clienteAtual['saldo']:
        messagebox.showerror('Um erro foi encontrado!', 'Seu saldo não é suficiente para \
        transferir esse valor. Realize um depósito e tente novamente.')
    else:
        server.send('5'.encode('utf-8'))
        dados = str({'conta' : conta_t.get(),
                     'conta2' : clienteAtual['conta'],
                     'valor' : valor_t.get(),
                    })
        dados = criptografar(dados)
        server.send(dados.encode('utf-8'))
        data = server.recv(1024).decode('utf-8')
        if data == 'n':
            messagebox.showerror('Um erro foi encontrado!', 'Não foi possível encontrar o cliente com a conta ' + conta_t.get() + '. Insira os dados corretamente. Nenhum valor foi debitado da sua conta.')
        elif data == 'ok':
            clienteAtual['saldo'] -= float(valor_t.get())
            conta_dados.configure(text = clienteAtual['conta'] +
            '\n\n\nR$ ' + str(clienteAtual['saldo']))
            messagebox.showinfo('Processo finalizado com sucesso!','Oba! Sua transferência para a conta ' + conta_t.get() + ' foi concluída com êxito!')
...
def Home():
    ...
    opc.add(cliente_t, text = 'Transferência')
    ...
    Label(cliente_t, text = 'Para realizar sua transferência,\ninsira os dados abaixo:', 
    bg = cor_p, fg = '#323232',font = (fonte_p, 35, 'bold')).pack(side = TOP, pady = 5)

    Label(cliente_t, text = 'CONTA DO BENEFICIÁRIO:', bg = cor_p, fg = 'white',
    font = (fonte_p, 20)).pack(side = TOP, pady = (35,0))
    conta_t.pack(side = TOP, pady = (10,0))

    Label(cliente_t, text = 'VALOR (R$):', bg = cor_p, fg = 'white',
    font = (fonte_p, 20)).pack(side = TOP, pady = (35,0))
    valor_t.pack(side = TOP, pady = (10,0))

    Label(cliente_t, text = 'SENHA:', bg = cor_p, fg = 'white',
    font = (fonte_p, 20)).pack(side = TOP, pady = (35,0))
    senha_t.pack(side = TOP, pady = (10,0))

    bt_t.pack(side = TOP, pady = 50)
...
cliente_cotas = Frame(opc, bg = cor_p)
cliente_t = Frame(opc, bg = cor_p)
...
senha_deposito = Entry(cliente_deposito, width = 30, show = '*')
...
senha_saque = Entry(cliente_saque, width = 30, show = '*')
...
conta_t = Entry(cliente_t, width = 30)
valor_t = Entry(cliente_t, width = 30)
senha_t = Entry(cliente_t, width = 30, show = '*')
bt_t = Button(cliente_t, text = 'TRANSFERIR', bd = '5', font = (fonte_p, 30, 'bold'),
fg = '#ffffff', bg = '#6f9596', relief = RIDGE, command = transferir)
